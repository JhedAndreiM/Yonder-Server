...
@foreach ($cartItems as $items)
  ...
  {{-- Insert divider ONCE when we reach the first out-of-stock item --}}
  @if(!$dividerShown && $available <= 0)
    @php $dividerShown = true; @endphp
    <div class="stock-divider">
      <hr>
      <h3 class="divider-text">Out of Stock Items</h3>
      <hr>
    </div>
  @endif
    ...
    <div class="card-details">
      ...
      <div class="div-quantity">
        <div class="quantity-controls">
          <button type="button" class="decrease" {{ $available <= 0 ? 'disabled' : '' }}>−</button>
          <input 
            type="number" 
            class="quantity" 
            value="{{ $available > 0 ? $items->quantity : 0 }}" 
            min="1" 
            max="{{ $available }}" 
            {{ $available <= 0 ? 'disabled' : '' }}>
          <button type="button" class="increase" {{ $available <= 0 ? 'disabled' : '' }}>+</button>
        </div>
      </div>
